...
async function getDeliveryConfig(): Promise<{ warehouses: Warehouse[]; couriers: Courier[] }> {
  ...
  if (!warehouses.ok || !couriers.ok) throw new Error("delivery_config_load_failed");
  return { warehouses: (await warehouses.json()).warehouses, couriers: (await couriers.json()).couriers };
}
async function createWarehouse(input: Record<string, unknown>): Promise<void> {
  ...
  if (!response.ok) throw new Error((await response.json().catch(() => null))?.error ?? "warehouse_create_failed");
}
async function createCourier(input: Record<string, unknown>): Promise<void> {
  ...
  if (!response.ok) throw new Error((await response.json().catch(() => null))?.error ?? "courier_create_failed");
}
async function setDefaultWarehouse(id: string): Promise<void> {
  ...
  if (!response.ok) throw new Error("warehouse_default_failed");
}
async function updateWarehouse(id: string, input: Record<string, unknown>): Promise<void> {
  ...
  if (!response.ok) throw new Error((await response.json().catch(() => null))?.error ?? "warehouse_update_failed");
}
async function updateCourier(id: string, input: Record<string, unknown>): Promise<void> {
  ...
  if (!response.ok) throw new Error((await response.json().catch(() => null))?.error ?? "courier_update_failed");
}
      ...
      <div className="delivery-columns">
        <div>
          ...
          {warehouses.map((warehouse) => editingWarehouse === warehouse.id ? <div className="delivery-item" key={warehouse.id}>
            ...
            <div className="button-row"><button type="button" onClick={() => void action(async () => updateWarehouse(warehouse.id, { name: (document.getElementById(`wh-name-${warehouse.id}`) as HTMLInputElement).value, address: (document.getElementById(`wh-address-${warehouse.id}`) as HTMLInputElement).value, latitude: Number((document.getElementById(`wh-lat-${warehouse.id}`) as HTMLInputElement).value), longitude: Number((document.getElementById(`wh-lon-${warehouse.id}`) as HTMLInputElement).value) }))} disabled={busy}>Save</button><button type="button" onClick={() => setEditingWarehouse(null)} disabled={busy}>Cancel</button></div>
        ...
        <div>
          ...
          {couriers.map((courier) => editingCourier === courier.id ? <div className="delivery-item" key={courier.id}>
            ...
            <div className="button-row"><button type="button" onClick={() => void action(() => updateCourier(courier.id, { name: (document.getElementById(`co-name-${courier.id}`) as HTMLInputElement).value, type: (document.getElementById(`co-type-${courier.id}`) as HTMLSelectElement).value, baseFeeMinor: Number((document.getElementById(`co-base-${courier.id}`) as HTMLInputElement).value), perKmRateMinor: Number((document.getElementById(`co-km-${courier.id}`) as HTMLInputElement).value), platformFeeMinor: Number((document.getElementById(`co-platform-${courier.id}`) as HTMLInputElement).value), surchargeMinor: Number((document.getElementById(`co-surcharge-${courier.id}`) as HTMLInputElement).value) }))} disabled={busy}>Save</button><button type="button" onClick={() => setEditingCourier(null)} disabled={busy}>Cancel</button></div>
...
function App() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { await login(accessCode); setAccessCode(""); setAuthenticated(true); } catch { setError("Access denied. Check the Admin Access Code."); } finally { setBusy(false); }
  }
