...
export async function settlePayment(db: D1Db, input: PaymentSettlementInput): Promise<{ orderId: string; state: "PAYMENT_CLEARED"; awardedPoints: number }> {
  ...
  const quote = { subtotalMinor: order.subtotal_minor, deliveryFeeMinor: order.delivery_fee_minor, discountMinor: order.discount_minor, storeCreditMinor: order.store_credit_minor, totalMinor: order.subtotal_minor - order.discount_minor - order.store_credit_minor + order.delivery_fee_minor, currency: order.currency };
  ...
  const loyalty = applyLoyaltyForPaidOrder({ pointsBalance: account.points_balance, lifetimePoints: account.lifetime_points, storeCreditMinor: account.store_credit_minor, tier: account.tier }, quote, { pointsPerMinor: config.pointsPerMinor, tierThresholds: config.tierThresholds, pointsPerCreditMinor: config.pointsPerCreditMinor });
  ...
  if (!existing) {
    statements.push(db.prepare("INSERT INTO loyalty_accounts (customer_id, points_balance, lifetime_points, store_credit_minor, tier, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(customer_id) DO UPDATE SET points_balance = excluded.points_balance, lifetime_points = excluded.lifetime_points, tier = excluded.tier, updated_at = excluded.updated_at")
      .bind(order.customer_id, loyalty.account.pointsBalance, loyalty.account.lifetimePoints, loyalty.account.storeCreditMinor, loyalty.account.tier, now.toISOString()));
    statements.push(db.prepare("INSERT INTO loyalty_transactions (id, customer_id, kind, points_delta, credit_delta_minor, reference_type, reference_id, created_at) VALUES (?, ?, 'earn', ?, 0, 'order_payment', ?, ?)")
