...
export async function settlePayment(db: D1Db, input: PaymentSettlementInput): Promise<{ orderId: string; state: "PAYMENT_CLEARED"; awardedPoints: number }> {
  ...
  const order = await db.prepare("SELECT id, customer_id, workflow_state, subtotal_minor, delivery_fee_minor, discount_minor, store_credit_minor, currency FROM orders WHERE id = ? LIMIT 1")
    .bind(input.orderId).first<{ id: string; customer_id: string; workflow_state: OrderWorkflowState; subtotal_minor: number; delivery_fee_minor: number; discount_minor: number; store_credit_minor: number; currency: string }>();
  if (!order) throw new Error("order_not_found");
  if (order.workflow_state === "PAYMENT_CLEARED") return { orderId: order.id, state: "PAYMENT_CLEARED", awardedPoints: 0 };
  ...
  const loyalty = applyLoyaltyForPaidOrder({ pointsBalance: account.points_balance, lifetimePoints: account.lifetime_points, storeCreditMinor: account.store_credit_minor, tier: account.tier }, {
    subtotalMinor: order.subtotal_minor, deliveryFeeMinor: order.delivery_fee_minor, discountMinor: order.discount_minor, storeCreditMinor: order.store_credit_minor, totalMinor, currency: order.currency,
  }, { pointsPerMinor: config.pointsPerMinor, tierThresholds: config.tierThresholds, pointsPerCreditMinor: config.pointsPerCreditMinor });
  ...
  if (referral && totalMinor >= config.referralMinimumOrderMinor && referral.referrer_customer_id !== referral.referred_customer_id) {
    ...
    statements.push(
      ...
      db.prepare("INSERT OR IGNORE INTO loyalty_transactions (id, customer_id, kind, points_delta, credit_delta_minor, reference_type, reference_id, created_at) SELECT ?, ?, 'referral', ?, 0, 'referral', ?, ? UNION ALL SELECT ?, ?, 'referral', ?, 0, 'referral', ?, ? WHERE changes() = 1").bind(crypto.randomUUID(), referral.referrer_customer_id, config.referrerPoints, referral.id, rewardTime, crypto.randomUUID(), referral.referred_customer_id, config.referredPoints, referral.id, rewardTime),
      db.prepare("INSERT INTO loyalty_accounts (customer_id, points_balance, lifetime_points, store_credit_minor, tier, updated_at) SELECT ?, ?, ?, 0, 'member', ? WHERE changes() = 2 UNION ALL SELECT ?, ?, ?, 0, 'member', ? WHERE changes() = 2 ON CONFLICT(customer_id) DO UPDATE SET points_balance = loyalty_accounts.points_balance + excluded.points_balance, lifetime_points = loyalty_accounts.lifetime_points + excluded.lifetime_points, tier = CASE WHEN loyalty_accounts.lifetime_points + excluded.lifetime_points >= ? THEN 'platinum' WHEN loyalty_accounts.lifetime_points + excluded.lifetime_points >= ? THEN 'gold' WHEN loyalty_accounts.lifetime_points + excluded.lifetime_points >= ? THEN 'silver' ELSE loyalty_accounts.tier END, updated_at = excluded.updated_at").bind(referral.referrer_customer_id, config.referrerPoints, config.referrerPoints, rewardTime, referral.referred_customer_id, config.referredPoints, config.referredPoints, rewardTime, config.tierThresholds.platinum, config.tierThresholds.gold, config.tierThresholds.silver),
    );
  }
