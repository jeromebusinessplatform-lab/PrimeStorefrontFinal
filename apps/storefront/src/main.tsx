...
function Checkout({ items, couriers, onComplete }: { items: Array<{ product: Product; quantity: number }>; couriers: Courier[]; onComplete: () => void }) {
  ...
  const [addressMessage, setAddressMessage] = useState("Enter an address; Geoapify autocomplete will be wired to suggestions.");
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      if (addressQuery.trim().length >= 3) setAddressMessage(`Searching nearby for “${addressQuery.trim()}”…`);
      else setAddressMessage("Enter at least 3 characters to search the delivery address.");
    }, 500);
  ...
  async function quoteDelivery() {
    ...
    if (!state.deliveryAddress.trim()) { setError("Enter the delivery address."); return; }
  ...
  return <section className="stack">
    ...
    <form className="panel stack" onSubmit={submit}>
      ...
      <h3>Address</h3><label className="field"><span>Search / Delivery Address</span><input required value={addressQuery} onChange={(e) => { setAddressQuery(e.target.value); setState({ ...state, deliveryAddress: e.target.value }); }} placeholder="Search Metro Manila or nearby provinces" /></label><p className="muted small-text">{addressMessage}</p><div className="inline-fields"><label className="field"><span>Latitude</span><input inputMode="decimal" value={state.latitude} onChange={(e) => setState({ ...state, latitude: e.target.value, deliveryQuoteId: "" })} /></label><label className="field"><span>Longitude</span><input inputMode="decimal" value={state.longitude} onChange={(e) => setState({ ...state, longitude: e.target.value, deliveryQuoteId: "" })} /></label></div>
      <div className="location-actions"><button type="button" onClick={() => setAddressMessage("Current-location request is ready; browser permission is required.")}>Use Current Location</button><button type="button" onClick={() => setAddressMessage("Drop-pin mode enabled; select a point on the map.")}>Drop Pin</button></div><div className="map-placeholder" role="img" aria-label="Delivery map preview">Map preview · selected address pin · zoom · drag/swipe</div>
      ...
      <label className="field"><span>Receipt Upload</span><input type="file" accept="image/*,.pdf" onChange={(e) => setState({ ...state, receiptObjectKey: e.target.files?.[0]?.name ?? "" })} /><span className="muted small-text">Taggun analysis is non-blocking; upload linkage is prepared for R2.</span></label>
...
function App() {
  ...
  useEffect(() => { if (!authenticated) return; void Promise.all([listProducts(), listCouriers()]).then(([nextProducts, nextCouriers]) => { setProducts(nextProducts); setCouriers(nextCouriers); }).catch(() => { setProducts([]); setCouriers([]); }).finally(() => setLoading(false)); }, [authenticated]);
